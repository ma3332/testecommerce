...
function Store({ paymentProcessor, TestERC20 }) {
  const buy = async item => {
    const response1 = await axios.get(`${API_URL}/api/getPaymentId/${item.id}`);

    const tx1 = await TestERC20.approve(paymentProcessor.address, item.price);  
    await tx1.wait();
  ...
  return (
    <ul className="list-group">
      <li className="list-group-item">
        Buy item1 - <span className='font-weight-bold'>100 TestToken</span>
        <button 
          type="button" 
      ...
      <li className="list-group-item">
        Buy item2 - <span className='font-weight-bold'>200 TestToken</span>
        <button 
          type="button" 
          className="btn btn-primary float-right"
          onClick={() => buy(ITEMS[1])}
